...
void PWM1_configurar(){
    ...
    PCLKCONbits.MCLKSEL = 0; //PWM Master Clock Selection bits: 00 = FOSC
    PCLKCONbits.DIVSEL = 1;  //PWM Clock Divider Selection bits: 01 = Divide ratio is 1:4
    ...
    PG1IOCONHbits.PMOD = 1; //01 = PWM Generator outputs operate in Independent mode
     ...
     PG1PER = FOSC*desired_period/PWM_DIVIDER - 1; // EL "-1" sale del datasheet
     PG1PHASE = 0;
     PG1DC = FOSC*minimum_duty/PWM_DIVIDER;
}
...
void set_angle(float angle){
    ...
    PG1DC = FOSC*required_duty/PWM_DIVIDER - 1; // EL "-1" sale del datasheet
