...
int main(void) {
    ...
    setup_MPU6050();
    while(1){
        if(MPU6050_new_data_available && (!MPU6050_calibrating) ){display_MPU6050_calibrated_data(); MPU6050_new_data_available = 0;}
        //if(MPU6050_new_data_available){display_MPU6050_raw_data(); MPU6050_new_data_available = 0;
        //if(!MPU6050_calibrating) {display_MPU6050_calibrated_data();display_MPU6050_offsets();}        }
        }
    return 0;
}
...
{
    /*Push button to enable the MPU6050 interrupt generator. Data will begin to flow from sensor*/
...
{
    /*MPU6050 interrupt generated. Data will be read*/
    
    //UART1_Write_Text("INT\n");
    MPU6050_read_accel_gyro(); 
    MPU6050_new_data_available = 1;
    if(MPU6050_calibrating){MPU6050_calibrate();}
    CNFBbits.CNFB11 = 0;
    IFS0bits.CNBIF  = 0;  
}
...
{
    /*Bus Collision detected in I2C line. Line used to communicate with MPU6050*/
...
{
    /*Event (as start, stop, ACK) occurred in I2C line. It doesn't mean that something went wrong. But we use the interruption to check if everything is fine*/
