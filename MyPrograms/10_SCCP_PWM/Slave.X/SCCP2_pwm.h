...
void set_REFCLK(){
    //Generate Reference Clock of 2MHz
    REFOCONLbits.ROOUT = 1; //0 = Reference clock external output is enabled. NEEDS TO BE ENABLED SO THAT SCCP2 WORKS!!
    REFOCONLbits.ROSEL = 2; //Reference Clock Source Select bits : 0010 = Primary Oscillator
    REFOCONHbits.RODIV = 2; //Base clock value divided by 4 (2 * 2)
    while(_ROSWEN){};       
